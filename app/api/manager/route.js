...
// Manager proposes a resolution — goes to pending, admin must confirm
export async function POST(req) {
  ...
  const { betId, outcome, action } = await req.json();

  // Admin can confirm or reject a manager proposal
  if (user.is_admin && action === 'confirm') {
    const { rows: propRows } = await sql`SELECT * FROM manager_proposals WHERE id = ${betId}`;
    if (!propRows[0]) return NextResponse.json({ error: 'Proposal not found' }, { status: 404 });
    const prop = propRows[0];

    // Resolve the bet
    const { rows: betRows } = await sql`SELECT * FROM bets WHERE id = ${prop.bet_id}`;
    ...
    await sql`UPDATE manager_proposals SET status = 'confirmed' WHERE id = ${betId}`;
    return NextResponse.json({ ok: true });
  }

  if (user.is_admin && action === 'reject') {
    await sql`UPDATE manager_proposals SET status = 'rejected' WHERE id = ${betId}`;
    return NextResponse.json({ ok: true });
  }

  // Manager submits proposal
  if (user.is_manager) {
